...
void remove_node(struct list_node** head, struct list_node* node_to_remove) {
    printf("Attempting to remove node with address: %p\n", (void*)node_to_remove);
    ...
    if (*head == node_to_remove) {
        *head = (*head)->next;
        printf("Removed head node. New head address: %p\n", (void*)*head);
        return;
    }
    ...
    if (current->next == NULL) {
        printf("Node to remove not found in the list.\n");
    }
    else {
        // Bypass the node to remove
        current->next = node_to_remove->next;
        printf("Node removed. Current node now points to: %p\n", (void*)current->next);
    }
}
...
void print_linked_list(struct list_node* head) {
    struct list_node* current = head;
    printf("Starting to print linked list...\n");

    while (current != NULL) {
        // Calculate the offset of 'node' in 'data_node'
        size_t offset = get_offset(&(((struct data_node*)0)->node), (void*)0);
        printf("Offset of 'node' in 'data_node': %zu bytes\n", offset);
        ...
        printf("Current list node address: %p\n", (void*)current);
        printf("Data node address derived from current list node: %p\n", (void*)data);
        printf("Data value in data node: %d\n", data->data);

        // Print the data
        printf("Data: %d\n", data->data);

        // Log the next node pointer
        printf("Next node pointer in current list node: %p\n", (void*)current->next);

        // Move to the next node
        current = current->next;
        printf("Moved to the next node: %p\n", (void*)current);
    }
